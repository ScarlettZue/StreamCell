      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-slate-900/80 border border-slate-700/60 rounded-xl pl-11 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-all"
            />
          </div>

          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center justify-center space-x-2 px-5 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 text-white rounded-xl text-sm font-semibold shadow-glow hover:opacity-95 transition-all"
          >
                ...
                clients?.map((client) => {
                  ...
                  return (
                    <tr key={client.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4 font-mono text-xs font-bold text-brand-300">{client.clientKey}</td>
                      <td className="px-6 py-4 font-semibold text-white">{client.name}</td>
                      <td className="px-6 py-4 font-mono text-xs text-slate-300">{client.phone}</td>
                      ...
                      <td className="px-6 py-4 text-xs text-slate-400">{formatDateCO(client.createdAt)}</td>
                      <td className="px-6 py-4 text-right">
                        {debt > 0 && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-600/30 rounded-lg text-xs font-semibold transition-all"
                          >
                            Registrar Pago
                          </button>
                        )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
            <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-slate-700 shadow-glass relative">
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Número de Celular</label>
                  <input
                    type="text"
                    required
                    value={phone}
                ...
                <div className="pt-3 flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded-xl bg-brand-600 text-white text-xs font-semibold shadow-glow hover:bg-brand-500 flex items-center space-x-1"
                  >
        ...
        {payModalOpen && selectedClient && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
            <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-slate-700 shadow-glass">
              <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-3">
                <h3 className="text-lg font-bold text-white flex items-center space-x-2">
              ...
              <form onSubmit={handlePaySubmit} className="space-y-4">
                <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800 text-xs">
                  <p className="text-slate-400">Cliente: <strong className="text-white">{selectedClient.name}</strong></p>
                  <p className="text-slate-400">Deuda actual: <strong className="text-rose-400">{formatCurrency(selectedClient.totalDebt)}</strong></p>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Monto Pagado ($)</label>
                  <input
                    type="number"
                    min="1"
                    step="500"
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500 font-mono font-bold"
                  />
                </div>
